...
func (k *K8sDiscoveryClient) DiscoverServices(serviceName string) []*ServiceInstance {
	...
	go func() {
		watcher, err := k.clientSet.CoreV1().Endpoints(k.namespace).Watch(context.TODO(), metav1.ListOptions{
			Watch: true,
		})
		if err != nil {
		...
		for event := range watcher.ResultChan() {
			...
			svc, err := k.clientSet.CoreV1().Services(k.namespace).Get(context.TODO(), endpoints.Name, metav1.GetOptions{})
			if err != nil {
				klog.Errorf("Failed to get service for endpoints %s: %v", endpoints.Name, err)
				continue
			}

			if svc.Labels[serviceName] == "" {
				continue
			}
